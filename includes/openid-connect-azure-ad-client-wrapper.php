...
class OpenID_Connect_Azure_AD_Client_Wrapper {
	...
	private $cookie_token_refresh_key = 'openid-connect-azure-ad-refresh';

	// user redirect cookie key
	public $cookie_redirect_key = 'openid-connect-azure-ad-redirect';
	 ...
	 * 
	 * @param \OpenID_Connect_Azure_AD_Client $client
	 * @param \OpenID_Connect_Azure_AD_Option_Settings $settings
	 * @param \OpenID_Connect_Azure_AD_Option_Logger $logger
	 */
	function __construct( OpenID_Connect_Azure_AD_Client $client, OpenID_Connect_Azure_AD_Option_Settings $settings, OpenID_Connect_Azure_AD_Option_Logger $logger ){
		$this->client = $client;
	 ...
	 * @param \OpenID_Connect_Azure_AD_Option_Logger $logger
	 *
	 * @return \OpenID_Connect_Azure_AD_Client_Wrapper
	 */
	static public function register( OpenID_Connect_Azure_AD_Client $client, OpenID_Connect_Azure_AD_Option_Settings $settings, OpenID_Connect_Azure_AD_Option_Logger $logger ){
		...
		add_filter( 'openid-connect-azure-ad-alter-request', array( $client_wrapper, 'alter_request' ), 10, 3 );
		...
		//Always use the alternate route for the callback url because of Azure AD URI constraints (no query string)
	...
	function get_end_session_logout_redirect_url( $redirect_url, $requested_redirect_to, $user ) {
		...
		$token_response = $user->get('openid-connect-azure-ad-last-token-response');
		if (! $token_response ) {
		...
		$claim = $user->get( 'openid-connect-azure-ad-last-id-token-claim' );
	...
	function authentication_request_callback() {
		...
		if ( ! $user ) {
			...
			do_action( 'openid-connect-azure-ad-update-user-using-current-claim', $user, $user_claim );
		}
		...
		if( $this->settings->redirect_user_back && !empty( $redirect_url ) ) {
			do_action( 'openid-connect-azure-ad-redirect-user-back', $redirect_url, $user );
	...
	function login_user( $user, $token_response, $id_token_claim, $user_claim, $subject_identity ){
		...
		update_user_meta( $user->ID, 'openid-connect-azure-ad-last-token-response', $token_response );
		update_user_meta( $user->ID, 'openid-connect-azure-ad-last-id-token-claim', $id_token_claim );
		update_user_meta( $user->ID, 'openid-connect-azure-ad-last-user-claim', $user_claim );
	...
	function get_user_by_identity( $subject_identity ){
		// look for user by their openid-connect-azure-ad-subject-identity value
		$user_query = new WP_User_Query( array(
			'meta_query' => array(
				array(
					'key'   => 'openid-connect-azure-ad-subject-identity',
	...
	private function get_username_from_claim( $user_claim ) {
		...
		if ( !empty( $this->settings->identity_key ) && isset( $user_claim[ $this->settings->identity_key ] ) ) {
			...
			$desired_username = $user_claim['email'];
		}
		else {
	...
	function create_new_user( $subject_identity, $user_claim ) {
		...
		$create_user = apply_filters( 'openid-connect-azure-ad-user-creation-test', TRUE, $user_claim );

		if ( ! $create_user ) {
			return new WP_Error( 'cannot-authorize', __( 'Can not authorize.' ), $create_user );
		}
		
		$user_claim = apply_filters( 'openid-connect-azure-ad-alter-user-claim', $user_claim );
		$user_data = array(
			'user_login' => $username,
		...
		$user_data = apply_filters( 'openid-connect-azure-ad-alter-user-data', $user_data, $user_claim );
		...
		add_user_meta( $user->ID, 'openid-connect-azure-ad-subject-identity', (string) $subject_identity, TRUE );

		// log the results
		$this->logger->log( "New user created: {$user->user_login} ($uid)", 'success' );

		// allow plugins / themes to take action on new user creation
		do_action( 'openid-connect-azure-ad-user-create', $user, $user_claim );
	...
	function update_existing_user( $uid, $subject_identity ) {
		...
		add_user_meta( $uid, 'openid-connect-azure-ad-subject-identity', (string) $subject_identity, TRUE );
		
		// allow plugins / themes to take action on user update
		do_action( 'openid-connect-azure-ad-user-update', $uid );
